...
class CVQueries {
    static async saveCVDataBatch(id_persona, cvData) {
        return await db.withTransaction(async (client) => {
            ...
            console.log(`🔄 Iniciando guardado para usuario: ${id_persona}`);

            try {
                // 1. Actualizar información personal
                if (cvData.informacion_personal) {
                    const personalData = cvData.informacion_personal;
                    const nombres = personalData.nombre_completo ? personalData.nombre_completo.split(' ') : [];
          ...
          `, [
                        ...
                        personalData.email, personalData.telefono, personalData.direccion,
                        personalData.linkedin, personalData.github
                    ]);
            ...
            DO UPDATE SET 
              ...
              fecha_actualizacion = CURRENT_TIMESTAMP
            RETURNING *
          `, [id_persona, profileData.puesto_actual, profileData.historial_profesional]);
              ...
              `, [id_persona, id_certificacion, new Date()]);
                ...
                // 4. Habilidades
                const allSkills = [
                    ...(cvData.habilidades?.tecnicas || []).map(s => ({ skill: s, categoria: 'TECNICA' })),
                    ...(cvData.habilidades?.blandas || []).map(s => ({ skill: s, categoria: 'BLANDA' }))
                ];
            ...
            `, [id_persona, id_habilidad, 3, new Date()]);
                ...
                console.log(`✅ Guardado completo:`, {
                    ...
                    certs: results.certifications.length,
